...
async def main():
    ceo = await create_agent_member(
        role="ceo", initial_prompt="you are ceo of a software game company"
    )
    ...
    agentGroup = AgentGroup(leader=ceo)

    await agentGroup.create_task(
        TaskRequestBody(
            input="create best shooter game in the world", additional_input={}
        )
    )
...
@pytest.mark.asyncio
@pytest.mark.vcr(
    "/home/mahdi/all/repositories/github.com/autogpt/AutoGPT/autogpts/autogpt/tests/vcr_cassettes/test_agent_group_happy_scenario/test_agent_group_happy_scenario.yaml"
)
async def test_agent_group_happy_scenario():
    empty_file(
        "/home/mahdi/all/repositories/github.com/autogpt/AutoGPT/autogpts/autogpt/agetn_group.db"
    )
    expected_uuids_for_tasks = [
    ...
    expected_uuids_for_agents = [
        "33333333-3333-3333-3333-333333333331",
        "33333333-3333-3333-3333-333333333332",
    ]
    with patch("forge.sdk.db.uuid4", side_effect=expected_uuids_for_tasks):
        with patch(
            "autogpt.agents.agent_member.uuid4", side_effect=expected_uuids_for_agents
        ):
            await main()
