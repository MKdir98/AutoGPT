...
class DivideAndConquerAgentPromptConfiguration(SystemConfiguration):
    ...
    DEFAULT_RESPONSE_SCHEMA = JSONSchema(
        ...
        properties={
            ...
            "commands": JSONSchema(
                description=(
                    "list of commands for tasks whose status is DOING. "
                    "The structure should be like this list[dict[name:str, args:object, task_id: str ]] "
                    "for complete list of tasks. Each command should be assign to a task wholse "
                    "status is DOING by task_id field. You need to choose some commands to progress tasks. "
                    "your list shouldn't be empty"
                ),
    ...
    def _generate_members_detail(self, agent:'AgentMember') -> list[str]:
        ...
        if agent.members:
            for emp in agent.members:
                result += "{agent_id: " + emp.id + ", description: " + emp.ai_profile.ai_role.rstrip('.') + "}\n"
        else:
...
def extract_command(
    ...
    try:
        ...
        for command_data in commands:
            command_obj = Command(command=command_data['name'], args=command_data.get("args", {}))
