...
HuggingChatEmbeddingParser = Callable[[Embedding], Embedding]
HuggingChatChatParser = Callable[[str], dict]


class HuggingChatModelName(str, enum.Enum):
...
hugging_chat_EMBEDDING_MODELS = {
    HuggingChatModelName.ADA: EmbeddingModelInfo(
        name=HuggingChatModelName.ADA,
        service=ModelProviderService.EMBEDDING,
        provider_name=ModelProviderName.HUGGINGCHAT,
...
hugging_chat_CHAT_MODELS = {
    info.name: info
    for info in [
        ChatModelInfo(
            name=HuggingChatModelName.GPT3,
        ...
        ChatModelInfo(
            name=HuggingChatModelName.GPT3_16k,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.HUGGINGCHAT,
        ...
        ChatModelInfo(
            name=HuggingChatModelName.GPT3_v3,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.HUGGINGCHAT,
        ...
        ChatModelInfo(
            name=HuggingChatModelName.GPT4,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.HUGGINGCHAT,
        ...
        ChatModelInfo(
            name=HuggingChatModelName.GPT4_32k,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.HUGGINGCHAT,
        ...
        ChatModelInfo(
            name=HuggingChatModelName.GPT4_v3,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.HUGGINGCHAT,
...
chat_model_mapping = {
    HuggingChatModelName.GPT3: [HuggingChatModelName.GPT3_v1, HuggingChatModelName.GPT3_v2],
    HuggingChatModelName.GPT3_16k: [HuggingChatModelName.GPT3_v2_16k],
    HuggingChatModelName.GPT4: [HuggingChatModelName.GPT4_v1, HuggingChatModelName.GPT4_v2],
    HuggingChatModelName.GPT4_32k: [
        HuggingChatModelName.GPT4_v1_32k,
        HuggingChatModelName.GPT4_v2_32k,
    ],
}
for base, copies in chat_model_mapping.items():
    for copy in copies:
        copy_info = ChatModelInfo(**hugging_chat_CHAT_MODELS[base].__dict__)
        copy_info.name = copy
        hugging_chat_CHAT_MODELS[copy] = copy_info
...
class HuggingChatConfiguration(ModelProviderConfiguration):
    fix_failed_parse_tries: int = UserConfigurable(3)
    pass


class HuggingChatCredentials(ModelProviderCredentials):
    """Credentials for HuggingChat."""

    email: SecretStr = UserConfigurable(from_env="HUGGING_CHAT_USERNAME")
    password: SecretStr = UserConfigurable(from_env="HUGGING_CHAT_PASSWORD")
    api_base: Optional[SecretStr] = UserConfigurable(
        default=None, from_env="hugging_chat_API_BASE_URL"
    )
    organization: Optional[SecretStr] = UserConfigurable(from_env="hugging_chat_ORGANIZATION")
    ...
    api_type: str = UserConfigurable(
        ...
        from_env=lambda: (
            ...
            else os.getenv("hugging_chat_API_TYPE")
        ),
    )
    api_version: str = UserConfigurable("", from_env="hugging_chat_API_VERSION")
    ...
    def _get_azure_access_kwargs(self, model: str) -> dict[str, str]:
        ...
        if model in hugging_chat_EMBEDDING_MODELS:
            return {"engine": deployment_id}
        else:
            return {"deployment_id": deployment_id}


class HuggingChatModelProviderBudget(ModelProviderBudget):
    graceful_shutdown_threshold: float = UserConfigurable()
    warning_threshold: float = UserConfigurable()


class HuggingChatSettings(ModelProviderSettings):
    configuration: HuggingChatConfiguration
    credentials: Optional[HuggingChatCredentials]
    budget: HuggingChatModelProviderBudget


class HuggingChatProvider(
    Configurable[HuggingChatSettings], ChatModelProvider, EmbeddingModelProvider
):
    default_settings = HuggingChatSettings(
        name="hugging_chat_provider",
        description="Provides access to HuggingChat's API.",
        configuration=HuggingChatConfiguration(
    ...
    _configuration: HuggingChatConfiguration

    def __init__(
        self,
        settings: HuggingChatSettings,
        logger: logging.Logger,
    ):
        assert settings.credentials, "Cannot create HuggingChatProvider without credentials"
        ...
        self._logger = logger
        self.chatbot = None
        # retry_handler = _HuggingChatRetryHandler(
        #     logger=self._logger,
    ...
    def get_token_limit(self, model_name: str) -> int:
        ...
        return hugging_chat_MODELS[model_name].max_tokens
    ...
    @classmethod
    def get_tokenizer(cls, model_name: HuggingChatModelName) -> ModelTokenizer:
        return tiktoken.encoding_for_model(model_name)

    @classmethod
    def count_tokens(cls, text: str, model_name: HuggingChatModelName) -> int:
    ...
    def count_message_tokens(
        ...
        model_name: HuggingChatModelName,
    ) -> int:
        ...
        if model_name.startswith("gpt-3.5-turbo"):
            ...
            raise NotImplementedError(
                ...
                " See https://github.com/hugging_chat/openai-python/blob/main/chatml.md for"
    ...
    async def create_chat_completion(
        ...
        **kwargs,
        ...
        """Create a completion using the HuggingChat API."""
        ...
        while True:
            ...
            for message in model_prompt:
                hugging_chatPrompt = hugging_chatPrompt + " " + message.role + ": " + message.content + "\n" 
            # hugging_chatPrompt += "your json response shouldn't be in block code. this is constraint.  just print pure json without any block code"
            #         "- If I asked something and it's type was string you can't pass null to it.\n" + \
            #         "- If you want send me the list in plan attribute don't put break line in middle of it. just put them all in one line \n"  \
            #         "- You can't ask question from user. Do your best. \n" \
            #         "- don't use sudo or su in shell commands\n" \
            # "- value of args field should be dictionary (json object with key and value) \n" \
            hugging_chatPrompt += "additional promt tip: "\
                    ...
                    "- Your json should be in text pure and valid json it will parse with json.loads() python so it should be valid. You can't wrap it in ``` or block code or ' or \". \n" \
            ...
            #         "- If you want send python code you should put \\t for indented code if code need that and please make sure it can run without compile error or something like that \n" \
            #         "- If you want to send code as string you need to escape characters like \" by adding \\. because if you don't do this, the text will not be accepted."
            response = self.chatbot.chat(hugging_chatPrompt, temperature=0, stream=False).wait_until_done()
            print("test8: " + response)
            if "```" in response:
                response_text = response.replace('```', '')
                json_obj_start = min(response_text.index('{'), response_text.index('['))
                response_text = response_text[json_obj_start:]
            else:
                response_text = response
            response_text = response_text.replace("\\_", "_").replace('\n', '')
            print("test9: " + response)    
            # response_text = response
            ...
            #     response_text = response
            response_args = {
                "model_info": hugging_chat_CHAT_MODELS[model_name],
                "prompt_tokens_used": len(hugging_chatPrompt),
                "completion_tokens_used": len(hugging_chatPrompt),
            }
            try:
                attempts += 1
                tool_calls = None
                if "create_agent" in response_text:
                    tool_calls = _tool_calls_compat_extract_calls(
                        response_text
                    )
                ...
                if attempts < self._configuration.fix_failed_parse_tries:
                    model_prompt = []
                    model_prompt.append(
                        ChatMessage.system(f"your json is wrong. please fix it and send it again. just send json (with no saying sorry or somehitng like that) ERROR PARSING YOUR RESPONSE:\n\n{e}")
                    )
                else:
                    raise
    ...
    async def create_embedding(
        ...
        **kwargs,
        ...
        """Create an embedding using the HuggingChat API."""
        embedding_kwargs = self._get_embedding_kwargs(model_name, **kwargs)
        response = await self._create_embedding(text=text, **embedding_kwargs)

        response_args = {
            "model_info": hugging_chat_EMBEDDING_MODELS[model_name],
    ...
    def _get_completion_kwargs(
        ...
        **kwargs,
        ...
        if functions:
            if hugging_chat_CHAT_MODELS[model_name].has_function_call_api:
...
# async def _create_embedding(text: str, *_, **kwargs) -> hugging_chat.Embedding:
#     """Embed text using the HuggingChat API.
...
#         str: The embedding.
#     """
#     return await hugging_chat.Embedding.acreate(
#         input=[text],
#         **kwargs,
#     )
...
# ) -> hugging_chat.Completion:
#     """Create a chat completion using the HuggingChat API.
...
#         for message in messages
#     ]
#     return await hugging_chat.ChatCompletion.acreate(
#         messages=raw_messages,
#         **kwargs,
#     )


class _HuggingChatRetryHandler:
    """Retry Handler for HuggingChat API call.
    ...
    _api_key_error_msg = (
        "Please double check that you have setup a PAID HuggingChat API Account. You can "
        "read more here: https://docs.agpt.co/setup/#getting-an-hugging_chat-api-key"
    )
...
def format_function_specs_as_typescript_ns(
    ...
    """Returns a function signature block in the format used by HuggingChat internally:
    https://community.hugging_chat.com/t/how-to-calculate-the-tokens-when-using-function-call/266573/18
    ...
    return (
        "namespace functions {\n\n"
        + "\n\n".join(format_hugging_chat_function_for_prompt(f) for f in functions)
        + "\n\n} // namespace functions"
    )


def format_hugging_chat_function_for_prompt(func: CompletionModelFunction) -> str:
    """Returns the function formatted similarly to the way HuggingChat does it internally:
    https://community.hugging_chat.com/t/how-to-calculate-the-tokens-when-using-function-call/266573/18
...
def count_hugging_chat_functions_tokens(
    functions: list[CompletionModelFunction], count_tokens: Callable[[str], int]
) -> int:
    """Returns the number of tokens taken up by a set of function definitions

    Reference: https://community.hugging_chat.com/t/how-to-calculate-the-tokens-when-using-function-call/266573/18  # noqa: E501
    """
    return count_tokens(
        "# Tools\n\n"
        "## functions\n\n"
