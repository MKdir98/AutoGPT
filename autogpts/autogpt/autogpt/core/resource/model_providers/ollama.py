...
ollama_EMBEDDING_MODELS = {
    OllamaModelName.ADA: EmbeddingModelInfo(
        name=OllamaModelName.ADA,
...
ollama_CHAT_MODELS = {
    ...
    for info in [
        ...
        ChatModelInfo(
            name=OllamaModelName.GPT4_TURBO,
            service=ModelProviderService.CHAT,
            provider_name=ModelProviderName.OLLAMA,
            prompt_token_cost=0.01 / 1000,
            completion_token_cost=0.03 / 1000,
...
chat_model_mapping = {
    OllamaModelName.GPT3: [OllamaModelName.GPT3_v1, OllamaModelName.GPT3_v2],
    OllamaModelName.GPT3_16k: [OllamaModelName.GPT3_v2_16k],
    OllamaModelName.GPT4: [OllamaModelName.GPT4_v1, OllamaModelName.GPT4_v2],
    OllamaModelName.GPT4_32k: [
        OllamaModelName.GPT4_v1_32k,
        OllamaModelName.GPT4_v2_32k,
    ],
    OllamaModelName.GPT4_TURBO: [
        OllamaModelName.GPT4_v3,
        OllamaModelName.GPT4_VISION,
    ],
}
for base, copies in chat_model_mapping.items():
    for copy in copies:
        copy_info = ChatModelInfo(**ollama_CHAT_MODELS[base].__dict__)
        copy_info.name = copy
        ollama_CHAT_MODELS[copy] = copy_info
...
class OllamaConfiguration(ModelProviderConfiguration):
...
class OllamaCredentials(ModelProviderCredentials):
    ...
    email: SecretStr = UserConfigurable(from_env="ollama_USERNAME")
    password: SecretStr = UserConfigurable(from_env="ollama_PASSWORD")
    api_base: Optional[SecretStr] = UserConfigurable(
        default=None, from_env="ollama_API_BASE_URL"
    )
    organization: Optional[SecretStr] = UserConfigurable(from_env="ollama_ORGANIZATION")
    ...
    api_type: str = UserConfigurable(
        ...
        from_env=lambda: (
            ...
            else os.getenv("ollama_API_TYPE")
        ),
    )
    api_version: str = UserConfigurable("", from_env="ollama_API_VERSION")
...
class OllamaSettings(ModelProviderSettings):
    ...
    credentials: Optional[OllamaCredentials]
    budget: OllamaModelProviderBudget


class OllamaProvider(
    Configurable[OllamaSettings], ChatModelProvider, EmbeddingModelProvider
):
    async def get_available_models(self) -> list[ChatModelInfo]:
        _models = (await self._client.models.list()).data
        return [OLLAMA_MODELS[m.id] for m in _models if m.id in OLLAMA_MODELS]
    ...
    @classmethod
    def get_tokenizer(cls, model_name: OllamaModelName) -> ModelTokenizer:
        return tiktoken.encoding_for_model(model_name)

    @classmethod
    def count_tokens(cls, text: str, model_name: OllamaModelName) -> int:
    ...
    def count_message_tokens(
        ...
        if model_name.startswith("gpt-3.5-turbo"):
            ...
            raise NotImplementedError(
                ...
                " See https://github.com/ollama/openai-python/blob/main/chatml.md for"
        ...
        **kwargs,
        ...
        while True:
            try:
                ...
                #     response_text = response
                response_args = {
                    "model_info": ollama_CHAT_MODELS[model_name],
                ...
                try:
                    attempts += 1
                    tool_calls = None
                    if 'Your job is to respond to a user-defined task' in model_prompt[0].content:
    ...
    def _get_completion_kwargs(
        ...
        **kwargs,
        ...
        if functions:
            if ollama_CHAT_MODELS[model_name].has_function_call_api:
...
# async def _create_embedding(text: str, *_, **kwargs) -> ollama.Embedding:
...
#         for message in messages
#     ]
#     return await ollama.ChatCompletion.acreate(
...
class _OllamaRetryHandler:
    ...
    _api_key_error_msg = (
        ...
        "read more here: https://docs.agpt.co/setup/#getting-an-ollama-api-key"
    )
...
def format_function_specs_as_typescript_ns(
    ...
    """Returns a function signature block in the format used by Ollama internally:
    https://community.ollama.com/t/how-to-calculate-the-tokens-when-using-function-call/266573/18
    ...
    return (
        "namespace functions {\n\n"
        + "\n\n".join(format_ollama_function_for_prompt(f) for f in functions)
        + "\n\n} // namespace functions"
    )


def format_ollama_function_for_prompt(func: CompletionModelFunction) -> str:
    """Returns the function formatted similarly to the way Ollama does it internally:
    https://community.ollama.com/t/how-to-calculate-the-tokens-when-using-function-call/266573/18
...
def count_ollama_functions_tokens(
    functions: list[CompletionModelFunction], count_tokens: Callable[[str], int]
) -> int:
    """Returns the number of tokens taken up by a set of function definitions

    Reference: https://community.ollama.com/t/how-to-calculate-the-tokens-when-using-function-call/266573/18  # noqa: E501
    """
    return count_tokens(
        "# Tools\n\n"
        "## functions\n\n"
